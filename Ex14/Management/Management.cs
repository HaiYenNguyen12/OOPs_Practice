...
namespace Ex14.Management
{
    public class Management
    {
        ...
        public List<Student> RecruitStudent(int num)
        {
            ...
            if (count > num)
            {
                var res = students.Where(x => x.GetType() == typeof(GoodStudent)).Cast<GoodStudent>().OrderByDescending(x => x.Gpa).ThenBy(y => y.FullName).ToList().Take(num);
            ...
            else if (count < num)
            {
                ...
                var res = students.Where(x => x.GetType() == typeof(NormalStudent)).Cast<NormalStudent>().OrderByDescending(x => x.EntryScore).ThenByDescending(y => y.EnglishScore).ThenBy(y => y.FullName).ToList().Take(remainder);
                foreach (var item in res)
                {
                    list.Add(item);
                }
